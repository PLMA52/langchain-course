...
from langchain_classic import \
    hub  # Load prompts from LangChain Hub (v1-compatible)
from langchain_classic.agents import \
    AgentExecutor  # Core agent execution engine
from langchain_classic.agents import \
    create_react_agent  # Modern import for creating a ReAct agent
# --- Runnable wrapper (THE FIX) ---
from langchain_core.runnables import \
    RunnableLambda  # Wraps any callable as a Runnable for clean tracing
...
llm = ChatOpenAI(
    model="gpt-4"
)  # Instantiates the LLM that will reason and decide actions

react_prompt = hub.pull(
    "hwchase17/react"
)  # Pulls the official ReAct prompt template from LangChain Hub
# This prompt enforces:
# Thought → Action → Observation → Final Answer format


agent = create_react_agent(
    llm, tools, react_prompt
)  # Creates a ReAct agent by combining the LLM (reasoning), tools (actions), and the ReAct prompt (Thought→Action→Observation loop)
...
)  # ✅ LangSmith will now show a replayable Runnable run
